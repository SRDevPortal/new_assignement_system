...
def _round_robin(candidates: list[frappe._dict]) -> frappe._dict:
	return sorted(candidates, key=lambda row: (row.last_assigned_at or "", row.current_open_leads or 0))[0]


def _balanced(candidates: list[frappe._dict]) -> frappe._dict:
	return sorted(
		candidates,
		key=lambda row: (row.current_open_leads or 0, row.last_assigned_at or "", row.agent),
	)[0]
...
def _weighted_balanced(candidates: list[frappe._dict]) -> frappe._dict:
	return sorted(
		candidates,
		key=lambda row: (_load_ratio(row), row.last_assigned_at or "", row.agent),
	)[0]
